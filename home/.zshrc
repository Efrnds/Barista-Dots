...
function agy-chats() {
  for d in ~/.gemini/antigravity-cli/brain/*; do
    if [ -d "$d" ] && [ -f "$d/.system_generated/logs/transcript.jsonl" ]; then
      id=$(basename "$d")
      title=$(jq -r 'select(.type=="USER_INPUT") | .content' "$d/.system_generated/logs/transcript.jsonl" 2>/dev/null | grep -A 1 "<USER_REQUEST>" | tail -n 1 | cut -c 1-60 | tr -d '\r\n')
      data=$(stat -c %y "$d/.system_generated/logs/transcript.jsonl" 2>/dev/null | awk '{print $1, $2}' | cut -d'.' -f1)
      echo -e "$data | \033[1;36m$id\033[0m | \033[1;33m$title...\033[0m"
    fi
  done | sort -r
}
